...
| T-050 | Cobertura de testes frontend (60%+) | Karma coverage ≥ 60%. Cypress e2e roda nos 4 fluxos críticos sem falha. | Alta | G | T-026, T-034, T-037 | ✅ (setup Karma/Jasmine do zero — `karma.conf.js`/`test.ts`/`tsconfig.spec.json`/target `test` no angular.json, ChromeHeadless real; 17 specs novos, **114 testes, 84.05% linhas** (meta 60% superada). Cypress: os 4 fluxos críticos passam — **auth, campaigns, flow-builder, inbox: 8/8 testes, 2 execuções consecutivas estáveis**. Bugs reais corrigidos no processo: `flow-builder.component.ts` usava `route.snapshot` (nunca reage a mudança de :id) → `route.paramMap.subscribe`; `cy.session()` não navega sozinho → `cy.visit` explícito após. `data-cy` mínimos adicionados em auth/campaigns/shell/inbox) |
